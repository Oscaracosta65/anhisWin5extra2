...
// Align extra-ball to the same calendar day as the main game.
    // This is more reliable than strict datetime equality because some rows may
    // store draw_date with a time component while others store date-only values.
    if (!empty($extraBallGameId)) {
        ...
        $mainDrawDay        = '';
        $mainDrawTs         = strtotime($drawDateStr);

        if ($mainDrawTs !== false) {
            $mainDrawDay = date('Y-m-d', $mainDrawTs);
        }

        $query = $db->getQuery(true)
            ->select($db->quoteName(['first', 'draw_date']))
            ->from($db->quoteName($dbCol))
        ...
        $db->setQuery($query);
        $extraBallRows = $db->loadObjectList();
        $extraBall = null;

        if (!empty($extraBallRows)) {
            foreach ($extraBallRows as $extraBallRow) {
                $extraDrawDate = isset($extraBallRow->draw_date) ? (string) $extraBallRow->draw_date : '';
                $extraDrawTs = strtotime($extraDrawDate);
                $extraDrawDay = ($extraDrawTs !== false) ? date('Y-m-d', $extraDrawTs) : '';

                if ($mainDrawDay !== '' && $extraDrawDay === $mainDrawDay) {
                    $extraBall = isset($extraBallRow->first) ? $extraBallRow->first : null;
                    break;
                }
            }

            if (($extraBall === null || $extraBall === '') && isset($extraBallRows[0]->first)) {
                $extraBall = $extraBallRows[0]->first;
            }
        }
...
foreach ($bonusSlots as $slotIndex => $slotCfg) {
    $slotRows = leFetchRowsByGameId($db, $dbCol, (string) $slotCfg['gameId'], 20);
    $slotValue = '';

    if ($drawDate !== '') {
        $mainDrawTs = strtotime((string) $drawDate);
        $mainDrawDay = ($mainDrawTs !== false) ? date('Y-m-d', $mainDrawTs) : '';

        foreach ($slotRows as $sr) {
            $bonusDrawDateRaw = (string) ($sr['draw_date'] ?? '');
            $bonusDrawTs = strtotime($bonusDrawDateRaw);
            $bonusDrawDay = ($bonusDrawTs !== false) ? date('Y-m-d', $bonusDrawTs) : '';

            if ($mainDrawDay !== '' && $bonusDrawDay === $mainDrawDay) {
